...
export class CinemaDetails implements OnInit {
  ...
  ngOnInit():void{const id=Number(this.route.snapshot.paramMap.get('id'));if(!id){this.error.set('Invalid cinema ID.');this.loading.set(false);return}forkJoin({cinema:this.cinemasService.getCinemaById(id),shows:this.showsService.getShows(),movies:this.movieService.getMovies()}).subscribe({next:r=>{this.cinema.set(r.cinema);this.movies.set(r.movies);this.shows.set(r.shows.filter(s=>s.cinema_id===id).filter(s=>new Date(`${s.show_date}T${s.show_time}`).getTime()>=Date.now()));this.loading.set(false)},error:()=>{this.error.set('Cinema schedule could not be loaded.');this.loading.set(false)}})}
